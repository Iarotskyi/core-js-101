...
function getPolynom(...constants) {
  ...
  return (x) => [...constants].reverse().reduce((acc, el, i) => acc + el * (x ** i), 0);
}
...
function logger(func, logFunc) {
  return (...x) => {
    try {
      logFunc.call(func, `${func.name}(${JSON.stringify(x).slice(1, -1)}) starts`);
      return func(...x);
    } finally {
      logFunc.call(func, `${func.name}(${JSON.stringify(x).slice(1, -1)}) ends`);
    }
  };
}
